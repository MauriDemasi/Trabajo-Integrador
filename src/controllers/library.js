...
const getLibrariesByCriteria= async (req, res) => {
  try {
    ...
    res.status(400).json({ action: "getLibrariesByCriteria", error: err.message });
  }
...
const updateLibraryById = async (req, res) => {
  try {
    ...
    const updatedLibrary = await libraryService.updateLibraryById(req.params.id, req.body);
...
const deleteLibraryById = async (req, res) => {
  try {
    ...
    const deletedLibrary = await libraryService.deleteLibraryById(req.params.id);
...
const createBookFromLibrary = async (req, res) => {
  try {
    ...
    const newBook = await libraryService.createBookFromLibrary(libraryId, bookData);
    res.status(201).json(newBook);
  } catch (error) {
    console.error('Error creating book:', error);
    res.status(400).json({ action: "createBookFromLibrary", error: error.message });
  }
